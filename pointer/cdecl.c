...
struct token_t {
 ...
 union {
  ...
  /**< if ::type is #ARRAY then it's the size of array (or -1 in case size has not been specified) */
  int  size;
 } info;
};
...
get_id(char *id)
{
 int i = 0;
 char *p = id;
 /* it's guaranteed by the caller that the first character in stdin is a letter */
...
get_token()
{
 ...
 if (c == EOF)
  ...
  if (
   ...
   token.type = TYPE;
  else if (strcmp(name, "struct") == 0 || strcmp(name, "enum")   == 0) {
   token.type = TYPE;

   skip_spaces();

   /* if structure or enum is defined in place then just skip the definition */
   int tmp = _getchar();
  ...
  } else if (strcmp(name, "const")    == 0 || strcmp(name, "volatile") == 0)
